...
class Processor(object):
    ...
    def __init__(self, args):
        ...
        if self.args.task in ['agnews']:
            ...
            self.num_label = 6
        elif args.task in ['mesh']:
            self.num_label = 11
        elif args.task in ['arxiv_cs']:
            self.num_label = 20
        elif args.task in ['arxiv_math']:
            self.num_label = 16
    ...
    def get_examples(self, mode, file_to_read = None):
        ...
        if mode == 'train':
            ...
            return self._create_examples(self.read_data(os.path.join(self.args.eval_dir, file_to_read)), mode)
...
def load_and_cache_examples(args, tokenizer, mode, size = -1, contra_name = None):
    ...
    if mode in ["test"]:
        ...
        if args.word_template == 0:
            cached_features_file = os.path.join(
                args.cache_dir,
                'cached_{}_{}_{}_{}_{}_{}_round{}_filter{}'.format(
                    mode,
                    args.task,
                    list(filter(None, args.model_name_or_path.split("/"))).pop(),
                    args.max_seq_len,
